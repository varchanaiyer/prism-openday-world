#!/usr/bin/env node
/* Builds the five rooms of the PRISM open day as WorkAdventure maps:
   lobby, poster hall, auditorium, team rooms, lounge. Each is a Tiled JSON
   map at one tile per cell (32 px), with the generated tileset embedded.
   Reads tilesets/prism.json, content/posters.json, content/teams.json.
   Writes maps/*.tmj and src/world.json.   node scripts/build-maps.cjs */
"use strict";
...
class Room {
  constructor(key, W, H) {
    this.key = key; this.W = W; this.H = H; const L = () => new Array(W * H).fill(0);
  ...
  floor(x0, y0, w, h, pair) { for (let y = y0; y < y0 + h; y++) for (let x = x0; x < x0 + w; x++) this.set("floor", x, y, pair[(x + y) % 2]); }
  ...
  startAt(x0, y0, w, h) { for (let y = y0; y < y0 + h; y++) for (let x = x0; x < x0 + w; x++) this.set("start", x, y, TS.T.START); }
  ...
  entry(name, x, y, w, h) { this.area(name, x, y, w, h, [prop("start", true)]); this.startAt(x, y, w, h); }
...
function lobby() {
  ...
  r.floor(1, 3, 30, 20, TS.floors.lobby); r.box(0, 0, 32, 24, 2);
...
/* ---------- the poster hall ---------- */
function posterHall() {
  const order = ["technical", "evals", "frontier", "governance"], rows = [];
  for (const t of order) { const ps = posters.filter(p => p.track === t); for (let i = 0; i < ps.length; i += 5) rows.push({ track: t, posters: ps.slice(i, i + 5) }); }
  const H = 4 + rows.length * 7 + 2, r = new Room("posters", 36, H);
  r.floor(1, 3, 34, H - 4, TS.floors.posters); r.box(0, 0, 36, H, 2);
  r.grid("walls", TS.banners.posters, 12, 1);
  rows.forEach((row, i) => {
    const y = 4 + i * 7;
    r.grid("props", TS.bays[row.track], 1, y + 1, true);
    r.zone("bay-" + row.track, 1, y, 34, 7);
    row.posters.forEach((p, k) => {
      const x = 7 + k * 5;
      r.grid("walls", TS.boards[p.id], x, y, true);
      r.floorOne(x, y + 3, 4, 1, TS.trackFloors[row.track]);
...
function stage() {
  ...
  r.floor(1, 3, 32, 26, TS.floors.stage); r.box(0, 0, 34, 30, 2);
...
function teamRooms() {
  ...
  r.floor(1, 3, 48, 40, TS.floors.teams); r.box(0, 0, 50, 44, 2);
  r.grid("walls", TS.banners.teams, 19, 1);
  const order = ["technical", "evals", "frontier", "governance"], sorted = [...teams].sort((a, b) => order.indexOf(a.track) - order.indexOf(b.track));
  sorted.forEach((t, i) => {
...
function lounge() {
  ...
  r.zone("lounge", 1, 3, 19, 18); r.zone("quiet", 21, 10, 8, 11);
...
const built = { lobby: lobby(), posters: posterHall(), stage: stage(), teams: teamRooms(), lounge: lounge() }, summary = [];
...
console.table(summary); console.log("posters hung:", posters.length, "· teams:", teams.length, "· pages base:", BASE);
